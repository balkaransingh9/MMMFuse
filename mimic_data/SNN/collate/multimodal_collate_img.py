...
class MultimodalCollate:
    ...
    def __call__(self, batch):
        ...
        if 'demographic' in self.modalities:
            demo = [torch.tensor(o['demographic'].values, dtype=torch.float32) for o in outs_list]
            col_names = outs_list[0]['demographic'].index
            seq_data['demographic']['values'] = torch.stack(demo, dim=0)
            seq_data['demographic']['feature_names'] = list(col_names)
