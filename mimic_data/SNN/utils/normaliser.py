...
def welford_update(curr_sample, n_old, m_old, s_old):
    batch_n = curr_sample.shape[0]
    n_new = n_old + batch_n

    delta = curr_sample - m_old
    m_new = m_old + delta.sum(dim=0) / n_new
    s_new = s_old + (delta * (curr_sample - m_new)).sum(dim=0)
    ...
    for sample in tqdm(train_samples):
        with env.begin(write=False) as txn:
            ...
            if retrieved_bytes is not None:
                ...
                x = loaded_data['sample']   # Tensor [time, features]

                if m is None:
                    m = torch.zeros(x.shape[1], dtype=x.dtype)
                    s = torch.zeros(x.shape[1], dtype=x.dtype)

                n, m, s = welford_update(x, n, m, s)

    stds = torch.sqrt(s / (n - 1))

    if ftype_data is not None:
        cont_mask = torch.tensor([1 if ftype_data[i] == 'continuous' else 0 
                                  for i in ftype_data['column_names']])
        mean = cont_mask * m
        std = stds.clone()
        std[cont_mask == 0] = 1
