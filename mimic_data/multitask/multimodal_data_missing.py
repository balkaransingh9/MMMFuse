...
class MultimodalData(Dataset):
    def __init__(
        ...
        extra = ['subject_id','stay','period_length','stay_id','original_split','ecg_path', 
                 'ecg', 'text', 'med', 'output', 'procedure', 'lab', 'vital', 'y_true_ihm', 'y_true_los']
            
        self.labels_phenotype = torch.tensor(self.data_split.drop(extra,axis=1).values).float()
        self.labels_ihm = torch.tensor(self.data_split['y_true_ihm'].values).float()
        self.labels_los = torch.tensor(self.data_split['y_true_los'].values).float()
    ...
    def __getitem__(self, idx):
        ...
        l_phenotype = self.labels_phenotype[idx]
        l_ihm = self.labels_ihm[idx]
        l_los = self.labels_los[idx]  

        return out, flags, l_phenotype, l_ihm, l_los
