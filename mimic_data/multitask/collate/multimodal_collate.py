...
class MultimodalCollate:
    ...
    def __call__(self, batch):
        outs_list, flags_list, labels_phenotype, labels_ihm, labels_los = zip(*batch)
        ...
        labels_phenotype = torch.stack(labels_phenotype, dim=0)
        labels_ihm = torch.tensor(labels_ihm, dtype=torch.float32).unsqueeze(1)
        labels_los = torch.tensor(labels_los).long()

        return {
            "inputs": seq_data,
            "present_mask": present_mask,
            "labels": {
                "phenotype": labels_phenotype,
                "in_hospital_mortality": labels_ihm,
                "length_of_stay": labels_los
            }
        }
